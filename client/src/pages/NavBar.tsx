    ...
    <nav className="bg-white shadow-lg w-full flex flex-col justify-between space-x-20 h-12">
      <div className="flex items-center space-x-20 py-4 px-4 mt-[-4px]">
        ...
        <div className="hidden md:flex space-x-5 text-lg mt-[-6px]">
          <button className={`nav-item flex items-center space-x-3 ${activeNavItem === 'Home' ? 'text-emerald-800' : 'text-black'}`} onClick={() => handleNavItemClick('Home')}>
          ...
          <button className={`nav-item flex items-center space-x-3 ${activeNavItem === 'LiveText' ? 'text-emerald-800' : 'text-black'}`} onClick={() => handleNavItemClick('LiveText')}>
            <FontAwesomeIcon icon={faCommentAlt} />
            <span>Live Text</span>
          </button>
          <button className={`nav-item flex items-center space-x-3 ${activeNavItem === 'Profile' ? 'text-emerald-800' : 'text-black'}`} onClick={() => handleNavItemClick('Profile')}>
